...
import { ArrowLeft, Calendar, Clock, Users, MapPin, ChevronRight, Utensils, X, Store } from "lucide-react"
...
export default function MyBookings() {
    ...
    const handleCancelBooking = async (booking) => {
        const confirmed = window.confirm(
            `Are you sure you want to cancel your table booking at ${booking.restaurant?.name || "this restaurant"}?`
        )
        if (!confirmed) return
        ...
        try {
            ...
            if (response.data.success) {
                // Update booking status locally without full refetch
                setBookings((prev) =>
                    prev.map((b) =>
    ...
    return (
        <AnimatedPage className="bg-slate-50 dark:bg-[#0a0a0a] min-h-screen pb-10 transition-colors">
            <div className="bg-white dark:bg-[#0a0a0a] p-4 flex items-center shadow-sm sticky top-0 z-10 border-b dark:border-gray-800">
                <button onClick={() => window.history.length > 1 ? navigate(-1) : navigate('/user/profile')}>
